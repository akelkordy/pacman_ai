...
class MultiPacmanAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.alphaBetaPruning(gameState, Directions.STOP, self.depth*gameState.getNumAgents(), 0, -10000000, 1000000)[0]
    
    def alphaBetaPruning(self, gameState, action, depth, index, alpha, beta):
        if depth == 0 or gameState.isLose() or gameState.isWin():
            return (action, self.evaluationFunction(gameState))
        elif index != 0:
            bestAction = (Directions.STOP, beta)
            nextLegalActions = gameState.getLegalActions(index)
            for nextLegalAction in nextLegalActions:
                nextState = gameState.generateSuccessor(index, nextLegalAction)
                currAction = self.alphaBetaPruning(nextState, action, depth - 1, (index + 1) % gameState.getNumAgents(), alpha, beta)
            ...
            for nextLegalAction in nextLegalActions:
                nextState = gameState.generateSuccessor(index, nextLegalAction)
                currAction = self.alphaBetaPruning(nextState, (nextLegalAction if depth is self.depth*gameState.getNumAgents() else action), depth - 1, 1, alpha, beta)
                if currAction[1] > bestAction[1]:
                    bestAction = currAction
